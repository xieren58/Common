...
object ImageUtil {
	...
	fun mosaicBitmap(context: Context, bitmap: Bitmap, mosaicMap: MutableMap<Pair<Float, Float>, Pair<Float, Float>>):Bitmap {
		val copy = bitmap.copy(bitmap.config, true)
		val bitmapMosaicCanvas = Canvas(copy)
		val paint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
			strokeWidth = UICommon.dip2px(context, 2F).toFloat()
		...
		mosaicMap.forEach {
			val xKey = it.key
			val yValue = it.value
			var startX = xKey.first
			var endX = xKey.second
			
			var startY = yValue.first
			var endY = yValue.second
			if (startX < 0) startX = 0F
			if (startX > width) startX = width.toFloat()
			if (endX < 0) endX = 0F
			if (endX > width) endX = width.toFloat()
			
			if (startY < 0) startY = 0F
			if (startY > width) startY = height.toFloat()
			if (endY < 0) endY = 0F
			if (endY > width) endY = height.toFloat()
			bitmapMosaicCanvas.drawLine(startX, startY, endX, endY, paint)
		}
		return copy
	}
}