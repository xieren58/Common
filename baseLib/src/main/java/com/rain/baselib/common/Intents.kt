...
class InternalStartContract(private val activity: Class<out Activity>) :
    ActivityResultContract<Array<out Pair<String, Any?>>, Intent>() {
    override fun createIntent(context: Context, input: Array<out Pair<String, Any?>>) =
        AnkoInternals.createIntent(context, activity, input)
...
class InternalStartNoIntentContract(private val activity: Class<out Activity>) :
    ActivityResultContract<Array<out Pair<String, Any?>>, Boolean>() {
    override fun createIntent(context: Context, input: Array<out Pair<String, Any?>>) =
        AnkoInternals.createIntent(context, activity, input)
...
inline fun <reified T : Activity> Fragment.createRegisterForActivity(crossinline block: (Intent?) -> Unit) =
    registerForActivityResult(InternalStartContract(T::class.java)) {
        block(it)
    }

inline fun <reified T : Activity> AppCompatActivity.createRegisterForActivity(crossinline block: (Intent?) -> Unit) =
    registerForActivityResult(InternalStartContract(T::class.java)) {
        block(it)
    }

inline fun <reified T : Activity> Fragment.createRegisterNoIntentActivity(crossinline block: (Boolean) -> Unit) =
    registerForActivityResult(InternalStartNoIntentContract(T::class.java)) {
        block(it)
    }

inline fun <reified T : Activity> AppCompatActivity.createRegisterNoIntentActivity(crossinline block: (Boolean) -> Unit) =
    registerForActivityResult(InternalStartNoIntentContract(T::class.java)) {
        block(it)
    }
...
fun Activity.navigationPopUpTo(home_id: Int, navigateId: Int) {
    ...
    findNavController.navigate(
        navigateId,
        null,
        NavOptions.Builder().setPopUpTo(findNavController.graph.id, true).build()
    )
}
