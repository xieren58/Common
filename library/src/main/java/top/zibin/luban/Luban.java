...
public class Luban implements Handler.Callback {
     ...
     */
    private File get(InputStreamProvider input, int compressSize, Context context) throws IOException {
        try {
            return new Engine(input, getImageCacheFile(context, Checker.SINGLE.extSuffix(input)), focusAlpha, compressSize).compress();
        } finally {
            input.close();
        }
    }
    ...
    private File compressReal(Context context, InputStreamProvider path) throws IOException {
        ...
        if (mCompressionPredicate != null) {
            if (mCompressionPredicate.apply(path.getPath())
                ...
                result = new Engine(path, outFile, focusAlpha, compressSize).compress();
            } else {
                result = new File(path.getPath());
            }
        } else {
            result = Checker.SINGLE.needCompress(mLeastCompressSize, path.getPath()) ?
                    new Engine(path, outFile, focusAlpha, compressSize).compress() :
    ...
    public static class Builder {
        ...
        public Builder setCompress(int compressSize) {
